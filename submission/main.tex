...
After constructing the seed instruction set, we employ GPT\mbox{-}5.1 to generate large-scale augmented variants for each instruction. For Level~1, we create 1,500 positive and 1,500 negative examples (500 short-answer and 1,000 multiple-choice each). For Level~2, we synthesize 500 positive and 500 negative examples (200 short-answer and 300 multiple-choice). For Level~3, we similarly produce 500 positive and 500 negative examples, as illustrated in Table \ref{tab:dataset_by_level} and Fig \ref{fig:sa_mc_level_distribution}. This augmentation process expands each seed into a diverse collection of semantically consistent variants while preserving the factual grounding of the original datasets.
...
The full dataset is randomly divided into training and testing subsets using an 80:20 split. The training subset is used exclusively for supervised fine-tuning, while the test subset is held out for evaluation on unseen instructions across all difficulty levels and task types.

\paragraph{Model Input Serialization}
To support instruction-tuned language models, each structured instruction record is serialized into a single linear text block prior to training.
This serialization preserves task-specific fields through explicit textual markers.
For short-answer instances, the serialized input includes Fact, Question, Output, and Explanation. For multiple-choice instances, Selections are flattened into a single string listing options A–D before being combined with the Question, Output, and Explanation. This unified representation ensures compatibility with standard fine-tuning pipelines while maintaining clear structural cues for different instruction types.
\newpage
\begin{lstlisting}[style=Matlab-editor]
[Short-answer only] Fact: [contextual fact]
